...
the Awesomebar. The form guides users through choosing a template Item and
entering the attribute values that should become variants.

## Variant Creation Tool

1. Select an Item template that is configured with exactly one variant
   attribute.
2. The tool automatically shows the attribute name and the allowed values.
3. Add rows to the table for each variant you would like to create. Optional
   columns allow you to override the generated Item Code, Item Name, SKU, and
